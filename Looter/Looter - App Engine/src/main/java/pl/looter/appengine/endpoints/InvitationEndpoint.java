...
import pl.looter.appengine.domain.Invitation;
...
public class InvitationEndpoint {
     ...
     * @throws NotFoundException if there is no {@code Invitation} with the provided ID.
     */
    @ApiMethod(
            name = "get",
            path = "invitation/{id}",
            httpMethod = ApiMethod.HttpMethod.GET)
    public Invitation get(@Named("id") Long id) throws NotFoundException {
        logger.info("Getting Invitation with ID: " + id);
        Invitation invitation = ofy().load().type(Invitation.class).id(id).now();
        if (invitation == null) {
            throw new NotFoundException("Could not find Invitation with ID: " + id);
        }
        return invitation;
    }

    /**
     * Inserts a new {@code Invitation}.
     */
    @ApiMethod(
            name = "insert",
            path = "invitation",
    ...
    public Invitation insert(Invitation invitation) {
        ...
        // You should validate that invitation.id has not been set. If the ID type is not supported by the
        // Objectify ID generator, e.g. long or String, then you should generate the unique ID yourself prior to saving.
        //
        // If your client provides the ID then you should probably use PUT instead.
        ofy().save().entity(invitation).now();
        logger.info("Created Invitation with ID: " + invitation.getId());

        return ofy().load().entity(invitation).now();
    }

    /**
     * Updates an existing {@code Invitation}.
     *
    ...
    public Invitation update(@Named("id") Long id, Invitation invitation) throws NotFoundException {
        // TODO: You should validate your ID parameter against your resource's ID here.
        checkExists(id);
        ofy().save().entity(invitation).now();
        logger.info("Updated Invitation: " + invitation);
        return ofy().load().entity(invitation).now();
    }

    /**
     * Deletes the specified {@code Invitation}.
     *
    ...
    public void remove(@Named("id") Long id) throws NotFoundException {
        checkExists(id);
        ofy().delete().type(Invitation.class).id(id).now();
        logger.info("Deleted Invitation with ID: " + id);
    }
    ...
    private void checkExists(Long id) throws NotFoundException {
        try {
            ofy().load().type(Invitation.class).id(id).safe();
